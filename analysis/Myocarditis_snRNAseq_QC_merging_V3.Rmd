...
```{r Table cluster for each condition}
# Table clusters for each condition
table(seuratM$clusterName, seuratM$pat_sub)
```


```{r Table cluster for each patient}
# Table cluster for each patient
table(seuratM$patient)
```
...
```{r FindAllMarkers function sorted by clusterName}

## marker genes by clusterName
Idents(seuratM) <- seuratM$clusterName
...
# Violin Plot BMP4 per condition
...
VlnPlot(seuratM, features = "ENSG00000125378.BMP4", pt.size = 0.01, group.by ="patient")
